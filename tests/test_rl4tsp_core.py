...
def test_diagnostic_summaries_include_confidence_intervals():
    ...
    assert "logprob_gap_spearman_r" in entropy_summary.columns
    assert "sample_variance_spearman_r" in entropy_summary.columns


def test_spearman_stats_detects_monotonic_nonlinear_relationship():
    ...
    assert stats["p"] <= 0.05
    assert stats["ci_low"] == pytest.approx(1.0)
    assert stats["ci_high"] == pytest.approx(1.0)


def test_spearman_stats_handles_constant_input():
    stats = _spearman_stats(pd.Series([1.0, 1.0, 1.0]), pd.Series([1.0, 2.0, 3.0]))

    assert stats == {"r": 0.0, "p": 1.0, "ci_low": 0.0, "ci_high": 0.0}
            ...
            [
                {
                    ...
                    "entropy_spearman_r": 0.1,
                    "entropy_spearman_r_ci_low": -0.1,
                    "entropy_spearman_r_ci_high": 0.3,
                    "logprob_gap_spearman_r": 0.2,
                    "logprob_gap_spearman_r_ci_low": 0.0,
                    "logprob_gap_spearman_r_ci_high": 0.4,
                    "sample_variance_spearman_r": 0.3,
                    "sample_variance_spearman_r_ci_low": 0.1,
                    "sample_variance_spearman_r_ci_high": 0.5,
                }
            ]
        ),
    )
