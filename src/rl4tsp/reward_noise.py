...
def plot_reward_noise_outputs(raw_csv: Path, summary_csv: Path, output_dir: Path) -> list[Path]:
    ...
    if not eval_summary.empty:
        methods = sorted(eval_summary["method"].unique())
        short_method = {
        ...
        n_cities_list = sorted(eval_summary["n_cities"].unique())
        ...
        for ax, method in zip(axes, methods):
            ...
            for idx, n_cities in enumerate(n_cities_list):
                subset = eval_summary[
                    (eval_summary["method"] == method)
                    & (eval_summary["n_cities"] == n_cities)
                ].sort_values("sigma_rel")
                if subset.empty:
                    continue
                yerr_col = (
                    "gap_percent_ci95"
                    if "gap_percent_ci95" in subset.columns
                    else "gap_percent_std"
                )
                ...
                ax.errorbar(
                    ...
                    label=f"n={int(n_cities)}",
                    **{k: v for k, v in style.items() if k != "linestyle"},
                )
            ax.set_title(short_method.get(method, method), fontweight="bold")
            ax.set_xlabel("Относительное стандартное отклонение шума награды")
            ax.legend(fontsize=8, loc="upper left")
            style_axis(ax)

        axes[0].set_ylabel("Отклонение при истинной оценке, %")
        fig.suptitle(
            "Качество при шумной обучающей награде: среднее и 95% ДИ",
            y=1.02,
            fontweight="bold",
        )
    ...
    if not train_raw.empty:
        ...
        methods = sorted(train_raw["method"].unique())
        short_method = {
            "Attention:greedy_rollout": "Attn",
            "PointerNet:greedy_rollout": "Ptr",
            "POMO": "POMO",
        ...
        for ax, method in zip(axes, methods):
            subset = train_raw[train_raw["method"] == method]
            epochs = sorted(subset["epoch"].unique())
            if len(epochs) > 60:
                step = max(1, len(epochs) // 60)
                epochs_sampled = epochs[::step]
            else:
                epochs_sampled = epochs

            sigma_rels = sorted(subset["sigma_rel"].unique())
            heatmap_data = np.zeros((len(sigma_rels), len(epochs_sampled)))
            for i, sigma in enumerate(sigma_rels):
                for j, epoch in enumerate(epochs_sampled):
                    val = subset[
                        (subset["sigma_rel"] == sigma)
                        & (subset["epoch"] == epoch)
                    ]["true_length_mean"].mean()
                    heatmap_data[i, j] = val

            im = ax.imshow(
                heatmap_data, aspect="auto", cmap='RdYlGn_r', interpolation="nearest"
            )

            tick_step = max(1, len(epochs_sampled) // 6)
            ax.set_xticks(range(0, len(epochs_sampled), tick_step))
            ax.set_xticklabels(
                [str(epochs_sampled[i]) for i in range(0, len(epochs_sampled), tick_step)],
                rotation=45, ha="right", fontsize=8,
            )
            ax.set_yticks(range(len(sigma_rels)))
            ax.set_yticklabels([f"{s:g}" for s in sigma_rels], fontsize=9)
            ax.set_title(short_method.get(method, method), fontweight="bold")
            ax.set_xlabel("Эпоха")

        axes[0].set_ylabel("σ_rel")
        cbar = fig.colorbar(im, ax=axes[-1], shrink=0.8)
        cbar.set_label("Длина тура", rotation=270, labelpad=15)
        fig.suptitle("Динамика качества обучения", y=1.02, fontweight="bold")
        fig.tight_layout(rect=[0, 0, 1, 0.95])
...
def summarize_reward_noise_raw(raw: pd.DataFrame) -> pd.DataFrame:
    raw = raw.copy()
    for column in [
