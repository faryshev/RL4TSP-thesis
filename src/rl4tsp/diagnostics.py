...
def _pearson_stats(x: pd.Series, y: pd.Series) -> dict[str, float]:
    ...
    return _correlation_stats_result(r, p_value, len(clean))


def _spearman_stats(x: pd.Series, y: pd.Series) -> dict[str, float]:
    clean = pd.DataFrame({"x": x, "y": y}).dropna()
    if len(clean) < 3 or clean["x"].std() == 0 or clean["y"].std() == 0:
        return {"r": 0.0, "p": 1.0, "ci_low": 0.0, "ci_high": 0.0}
    if scipy_stats is None:
        r = float(clean["x"].rank(method="average").corr(clean["y"].rank(method="average")))
        p_value = 1.0
    else:
        result = scipy_stats.spearmanr(clean["x"], clean["y"])
        r = float(result.statistic)
        p_value = float(result.pvalue)
    return _correlation_stats_result(r, p_value, len(clean))


def _correlation_stats_result(r: float, p_value: float, count: int) -> dict[str, float]:
    if not np.isfinite(r):
        return {"r": 0.0, "p": 1.0, "ci_low": 0.0, "ci_high": 0.0}
    if not np.isfinite(p_value):
        p_value = 1.0
    r = float(np.clip(r, -1.0, 1.0))
    if count <= 3 or abs(r) >= 1.0:
        return {"r": r, "p": p_value, "ci_low": r, "ci_high": r}
    z_value = np.arctanh(r)
    delta = 1.96 / math.sqrt(count - 3)
    return {
        "r": r,
        "p": p_value,
...
def plot_noise_summary(summary: pd.DataFrame, output_path: Path) -> Path:
    ...
    ax.legend()
    style_axis(ax)
    ax.set_xticks([0, 1, 2, 5, 10, 15, 20, 25, 30, 40, 50])
    fig.tight_layout()
    fig.savefig(output_path, dpi=180)
    plt.close(fig)
    return output_path
...
def plot_entropy_correlations(summary: pd.DataFrame, output_path: Path, *, correlation: str = "pearson") -> Path:
    ...
    for ax, (prefix, title) in zip(axes.ravel(), signals):
        if correlation == "pearson":
            ...
            r_col = f"{prefix}_spearman_r"
            low_col = f"{prefix}_spearman_r_ci_low"
            high_col = f"{prefix}_spearman_r_ci_high"
            correlation_label = "Spearman ρ"
        for method in sorted(summary["method"].unique()):
        ...
        ax.axhline(-0.5, color=PALETTE["grid"], linestyle="--", linewidth=1)
        ax.set_title(f"{title}: {correlation_label} и 95% ДИ")
        ax.set_xlabel("Число городов")
        ax.set_ylabel(f"{correlation_label} с отклонением")
        ax.set_ylim(-1.05, 1.05)
        ax.legend(fontsize=8)
        style_axis(ax)
        #add_ci95_note(ax, f"Вертикальные интервалы показывают 95% доверительный интервал для {correlation_label}.")
    fig.tight_layout()
    fig.savefig(output_path, dpi=180)
    plt.close(fig)
    return output_path
...
def plot_entropy_from_csv(raw_csv: Path, summary_csv: Path, output_dir: Path) -> dict[str, str]:
    ...
    spearman_path = output_dir / "entropy_spearman_correlations.png"
    plot_entropy_raw(raw, summary, gap_path)
    plot_entropy_correlations(summary, correlations_path, correlation="pearson")
    plot_entropy_correlations(summary, spearman_path, correlation="spearman")
    return {
        "entropy_plot": str(gap_path),
        "entropy_correlations_plot": str(correlations_path),
        "entropy_spearman_correlations_plot": str(spearman_path),
    }
...
def save_diagnostics_outputs(
    *,
    ...
    if entropy_raw is not None and entropy_summary is not None:
        ...
        plot_sources["entropy_spearman_correlations_plot"] = {"summary_csv": str(summary_path)}
